...
if ( ! class_exists( 'YITH_Stripe_Connect' ) ) {
	...
	class YITH_Stripe_Connect {
		 ...
		 *
		 * @param $methods array Array of available gateways
		 *
		...
		public function get_gateway() {
			if ( ! $this->gateway instanceof WC_Payment_Gateway_CC ) {
		 ...
		 *
		 * @param $available_gateways array Array of available gateways
		 *
		 * @return array Filtered array of gateways
		 */
		...
		public function set_default_token_on_stripe( $token_id, $token = null ) {
			...
			try {
				...
				// delete card
				$customer = $gateway->api_handler->update_customer( $customer_sc['id'], array_merge(
					array(
