...
if ( ! function_exists( 'yith_wcsc_check_free_order_item' ) ) {
	function yith_wcsc_check_free_order_item( $order_item ) {
		$item_free  = true;
...
if ( ! function_exists( 'yith_wcsc_prepare_commission_args' ) ) {
	...
	function yith_wcsc_prepare_commission_args( $commission = array(), $index = '' ) {
		...
		$prepared_commission = array(
			...
			'cost'                      => $line_total . get_woocommerce_currency_symbol(),
			'commission_text_detail'    => $commission_text_detail,
			'day_delay'                 => $day_delay,
			'commission_status'         => $commission['commission_status'],
			'commission_rate'           => $commission['commission_rate'],
			'commission_type'           => $commission['commission_type'],
			'commission_total'          => $commission['commission'] . get_woocommerce_currency_symbol(),
		...
		switch ( $status ) {
			case 'sc_pending':
				...
				if ( 'disconnect' == $receiver ) {
					$status_text = ( 'resumed' == $context ) ? __( 'Disconnected', 'yith-stripe-connect-for-woocommerce' ) : __( 'The user\'s Stripe
					account has been disconnected', 'yith-stripe-connect-for-woocommerce' );
			...
			case 'sc_transfer_error':
				$status_text = ( 'resumed' == $context ) ? __( 'Transfer error...', 'yith-stripe-connect-for-woocommerce' ) : __( 'Error processing
				transfer', 'yith-stripe-connect-for-woocommerce' );;

				return $status_text;
				break;
			case 'sc_transfer_success':
				$status_text = ( 'resumed' == $context ) ? __( 'Paid', 'yith-stripe-connect-for-woocommerce' ) : __( 'Commission paid',
					'yith-stripe-connect-for-woocommerce' );
