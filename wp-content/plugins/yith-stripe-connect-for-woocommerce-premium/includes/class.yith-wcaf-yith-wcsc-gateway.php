			...
			foreach ( $payments_id as $payment_id ) {
				...
				if ( ! empty( $destination ) ) {
					...
					if ( isset( $transfer['error_transfer'] ) ) {
						...
						// Prepare message
						$error_message = __( 'Please take a look at Stripe Connect log file for more details.',
							'yith-stripe-connect-for-woocommerce' );

						// Display messages on order note and log file
						$stripe_connect_gateway->log( 'error', __( 'Affiliates\' payment: ', 'yith-stripe-connect-for-woocommerce' ) .
